...
| 지표 | 낮음 | 보통 | 좋음 | 매우 좋음 |
|------|:----:|:----:|:----:|:---------:|
| **충실도** (Faithfulness) | ~40% | 40~60% | 60~80% | **80%+** |
| **답변 연관성** (Answer Relevancy) | ~50% | 50~70% | 70~85% | **85%+** |
| **컨텍스트 정밀도** (Context Precision) | ~40% | 40~65% | 65~80% | **80%+** |
| **컨텍스트 재현율** (Context Recall) | ~50% | 50~70% | 70~85% | **85%+** |

**충실도**가 가장 우선순위가 높습니다. LLM이 문서 밖의 내용을 지어내는지(Hallucination)를 측정하는 지표로, 60% 미만이면 답변 신뢰도가 낮다고 판단합니다.

**컨텍스트 정밀도**가 낮게 나올 경우 → FAISS 검색 파라미터(`k` 값 축소, 청크 크기 조정) 점검이 필요합니다.

#### ⚠️ 이 프로젝트 평가의 한계

이 프로젝트는 **키워드 기반 근사 평가**를 사용하므로 정식 평가 도구(RAGAS 등)보다 수치가 낮게 산출되는 경향이 있습니다.

| 구분 | 방식 | 정확도 | 비용 |
|------|------|--------|------|
| **이 프로젝트** | 불용어 제거 + 한국어 어근 매칭 | 근사치 | 무료 |
| **RAGAS** | LLM이 직접 의미론적 평가 | 높음 | API 비용 발생 |

따라서 수치의 **절대값보다는 모델 간 상대 비교** 또는 **시간에 따른 추세 변화**를 파악하는 용도로 활용하는 것이 적합합니다.
